 ...
 * item页底部的推荐商品适配器
 */
public class GoodsDetailRecommendAdapter implements Holder<List<RecommendGoodsBean>> {
    private View rootview;
    private GridView gv_recommend_goods;

    @Override
    public View createView(final Context context) {
        rootview = ((LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.item_goodsdetail_recommend, null);
    ...
    public void UpdateUI(final Context context, int position, final List<RecommendGoodsBean> data) {
        gv_recommend_goods.setAdapter(new GoodsDetailRecommendItemAdapter(context, data));
    }
}
